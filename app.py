...
def display_comparison_table(per_word_comparison: List[Dict], show_only_errors: bool = False):
    ...
    if show_only_errors:
        data = [item for item in per_word_comparison if not item['match']]
    else:
    ...
    styled_df = df[['word', 'ref_phonemes', 'rec_phonemes', 'Status']].style.apply(highlight_errors, axis=1)
...
def main():
    ...
    with col1:
        st.markdown("**Click the microphone to start/stop recording (max 30 seconds)**")
